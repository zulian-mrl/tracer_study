...
@section('title', \App\Models\Setting::get('login_judul_browser', 'Login Admin - Tracer Study'))
    ...
    <div class="w-full max-w-md float-3d fade-up">
        <div class="glass rounded-2xl overflow-hidden border-white/10 shadow-2xl">
            ...
            <div class="p-6 text-center border-b border-white/10 bg-gradient-to-br from-slate-800/80 to-slate-900/40">
                <div class="text-5xl mb-2 lock-3d">{{ \App\Models\Setting::get('login_ikon', '🔐') }}</div>
                <h1 class="text-2xl font-extrabold tracking-wide grad-text uppercase">{{ \App\Models\Setting::get('login_judul', 'Login Admin') }}</h1>
                <p class="text-sm text-gray-400 mt-1">{{ \App\Models\Setting::get('login_subjudul', 'Tracer Study LPKM UMMY Solok') }}</p>
            </div>
            ...
            <div class="p-6 space-y-4">
                ...
                <form action="{{ route('login') }}" method="POST" class="space-y-4">
                    ...
                    <div>
                        <label class="block text-xs font-semibold uppercase tracking-wider text-gray-400 mb-1">{{ \App\Models\Setting::get('login_label_email', 'Email') }}</label>
                    ...
                    <div>
                        <label class="block text-xs font-semibold uppercase tracking-wider text-gray-400 mb-1">{{ \App\Models\Setting::get('login_label_password', 'Kata Sandi') }}</label>
                        <div class="relative">
                    ...
                    <label class="flex items-center space-x-2 text-sm text-gray-300">
                        ...
                        <span>{{ \App\Models\Setting::get('login_ingat_saya', 'Ingat saya') }}</span>
                    </label>
                ...
                @if($dariRiwayat)
                    <a href="{{ route('login', ['kembali' => 1]) }}"
                       class="block text-center text-sm bg-slate-700/70 hover:bg-slate-600 border border-slate-500/50 px-4 py-2 rounded-lg text-gray-200 transition">
                        {{ \App\Models\Setting::get('login_link_kembali', '⬅️ Kembali ke Halaman Login') }}
                    </a>
                @endif

                <a href="{{ route('kuesioner.index') }}" class="block text-center text-sm text-gray-400 hover:text-amber-400 transition">
                    {{ \App\Models\Setting::get('login_link_kuesioner', '← Kembali ke Form Kuesioner') }}
                </a>
            </div>
        </div>
    </div>
