...
@section('title', 'Kelola Akun Admin - Tracer Study')
@section('judul', '👥 Kelola Akun Admin')
                            ...
                            <tr class="border-b border-slate-700/40 hover:bg-slate-800/30 transition">
                                ...
                                <td class="py-3 text-right whitespace-nowrap">
                                    ...
                                    @elseif(!$a->is_super)
                                        ...
                                        <form action="{{ route('akun.hapus', $a) }}" method="POST" class="inline form-hapus"
                                              data-akun="{{ $a->email }}">
                                            @csrf
    ...
    <script>
        document.querySelectorAll('form.form-hapus').forEach(function (form) {
            form.addEventListener('submit', function (e) {
                if (!window.confirm('Yakin hapus akun ' + form.dataset.akun + '?')) {
                    e.preventDefault();
                }
            });
        });
